  ...
  try {
    ...
    try {
      ...
      if (patients.length > 0) {
        console.log(`Found patient: ${patients[0].FirstName} ${patients[0].LastName}`);
      ...
      console.error('❌ Error fetching patient data:', error instanceof Error ? error.message : error);
    }
    ...
    try {
      ...
      console.error('❌ Error adding treatment note:', error instanceof Error ? error.message : error);
      console.log('💡 Note: Make sure to use valid patient, case, and practitioner IDs\n');
    }
    ...
    try {
      const treatmentPlan = [
        {
          ...
          notes: 'Initial assessment completed. Patient presents with chronic lower back pain. Developed treatment plan focusing on strengthening exercises and manual therapy.'
        },
        {
          patientId: 123,
          caseId: 456,
          ...
          notes: 'Session 2: Introduced core strengthening exercises. Patient responded well to manual therapy. Pain level decreased from 7/10 to 5/10.'
        },
        {
          patientId: 123,
          caseId: 456,
          ...
          notes: 'Session 3: Patient showing excellent progress. Pain reduced to 3/10. Increased exercise difficulty. Patient educated on home exercise programme.'
        }
      ];
      ...
      for (const [index, note] of treatmentPlan.entries()) {
        try {
          ...
          console.log(`✅ Added treatment note ${index + 1}/${treatmentPlan.length}`);

          // Small delay between requests to be respectful to the API
          await new Promise(resolve => setTimeout(resolve, 500));
        } catch (error) {
          console.error(`❌ Error adding note ${index + 1}:`, error instanceof Error ? error.message : error);
        }
      }
      console.log('\n');
    } catch (error) {
      console.error('❌ Error in bulk note addition:', error instanceof Error ? error.message : error);
    }
    ...
    try {
      ...
      console.log(`Found ${practitionerPatients.length} patients for practitioner:`);
      ...
      console.error('❌ Error fetching practitioner patients:', error instanceof Error ? error.message : error);
    }
    ...
    console.error('💥 Fatal error in treatment notes demo:', error instanceof Error ? error.message : error);
    process.exit(1);
  }
}
...
async function addCurrentTreatmentNote(
  ...
  try {
    ...
    console.log(`✅ Added note for patient ${patientId}: "${notes.substring(0, 50)}..."`);
    return result;
  } catch (error) {
    console.error(`❌ Failed to add note for patient ${patientId}:`, error instanceof Error ? error.message : error);
    throw error;
  }
}
