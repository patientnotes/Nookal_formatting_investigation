...
async function testNookalStyleImages() {
  ...
  console.log("Theory: Alt text must match a substring of the base64 image data\n");
  ...
  try {
    ...
    for (let i = 0; i < testCases.length; i++) {
      ...
      console.log(`   With Nookal Images: "${testCase.withNookalImage.substring(0, 80)}${testCase.withNookalImage.length > 80 ? '...' : ''}"`);
      ...
      try {
        ...
        const ourNote = allNotes.find(note => {
          if (note.answers && Array.isArray(note.answers)) {
            ...
            if (firstAnswer && firstAnswer.answers && Array.isArray(firstAnswer.answers)) {
              return firstAnswer.answers[0] && firstAnswer.answers[0].includes(`[NOOKAL IMG ${i + 1}]`);
            }
          }
          return false;
        });
        ...
        if (ourNote) {
          ...
          console.log(`   Retrieved: "${retrievedText.substring(0, 100)}${retrievedText.length > 100 ? '...' : ''}"`);

          const noteContent = retrievedText.replace(`[NOOKAL IMG ${i + 1}] `, '');

          // Check if Nookal-style images were preserved
          if (noteContent.includes('<img src="data:image/gif;base64,') && noteContent.includes('alt=')) {
            console.log(`   🎉 EXCELLENT! Nookal-style images preserved perfectly!`);

            // Count preserved images
            const imageCount = (noteContent.match(/<img[^>]*>/g) || []).length;
            console.log(`   📊 ${imageCount} image(s) preserved with proper alt text`);
            successCount++;
          }
          // Check if images were partially preserved
          else if (noteContent.includes('<img') || noteContent.includes('data:image')) {
    ...
    console.log("══════════════════════════════════");
    console.log(`🎉 Perfect (images work): ${successCount}/${testCases.length}`);
    console.log(`✅ Partial (some success): ${partialCount}/${testCases.length}`);
    console.log(`❌ Failed: ${failCount}/${testCases.length}`);
    const totalSuccess = successCount + partialCount;
    console.log(`📈 Success Rate: ${Math.round((totalSuccess / testCases.length) * 100)}%\n`);
    ...
    if (successCount === testCases.length) {
      ...
      console.log("💡 Consider requesting Nookal's official image API documentation");
    }
    ...
    Object.entries(nookalStyleImages).forEach(([key, img]) => {
      ...
      console.log(`${key}: Alt text found in base64: ${altInBase64 ? '✅' : '❌'}`);
      console.log(`   Alt: "${img.alt}"`);
      console.log(`   Base64 length: ${base64Data.length}, Alt length: ${img.alt.length}`);
    });
