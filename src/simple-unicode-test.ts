...
async function simpleUnicodeTest() {
  ...
  try {
    ...
    const ourNote = retrievedNotes.find(note =>
      note.Notes && note.Notes.includes("[SIMPLE TEST]")
    );
    ...
    if (ourNote) {
      ...
      if (ourNote.Notes === testNote) {
        ...
        console.log(`Length diff: ${testNote.length} vs ${ourNote.Notes.length}`);

        // Show character by character comparison
        for (let i = 0; i < Math.max(testNote.length, ourNote.Notes.length); i++) {
          const orig = testNote[i] || 'END';
          const retr = ourNote.Notes[i] || 'END';
          if (orig !== retr) {
            console.log(`  Diff at pos ${i}: "${orig}" (${orig.charCodeAt(0)}) vs "${retr}" (${retr.charCodeAt(0)})`);
          }
        }
      }
    } else {
      ...
      const foundInAll = allNotes.find(note =>
        note.Notes && note.Notes.includes("[SIMPLE TEST]")
      );
      ...
      if (foundInAll) {
        ...
        console.log(`Patient ID mismatch? Our patient: ${patientId}, Note's patient: ${foundInAll.PatientID}`);
      } else {
